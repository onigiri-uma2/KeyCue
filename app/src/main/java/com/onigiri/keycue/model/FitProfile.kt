 ...
 *
 * @param keyCenters 各キーそれぞれの正規化中心座標（要素数は [GameProfileRegistry.current.keyCount] と一致）
...
data class FitProfile(
    ...
    init {
        ...
        require(keyCenters.size == expectedKeyCount) {
            "FitProfile must contain exactly $expectedKeyCount key centers, but got ${keyCenters.size}"
        }
